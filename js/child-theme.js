	...
	document.addEventListener('DOMContentLoaded', () => {
	  ...
	  navs.forEach(nav => {
	    ...
	    const getLabel = (toggle, expanded) => {
	      ...
	      if (!current) return '';
	      if (expanded) {
	        return current.replace(/^Rozwiń podmenu:/, 'Zwiń podmenu:').replace(/^Expand submenu:/, 'Collapse submenu:');
	      }
	      return current.replace(/^Zwiń podmenu:/, 'Rozwiń podmenu:').replace(/^Collapse submenu:/, 'Expand submenu:');
	    };
