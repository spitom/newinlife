...
if ( ! function_exists( 'inlife_get_post_card_image_id' ) ) {
    ...
    function inlife_get_post_card_image_id( $post_id ) {
        ...
        $custom_image = function_exists( 'get_field' ) ? get_field( 'post_card_image', $post_id ) : 0;
...
if ( ! function_exists( 'inlife_get_post_target_link' ) ) {
    /**
     * Returns post target link data.
     *
     * If ACF field post_redirect_url is set, it is used instead of the post permalink.
     * Supports both ACF Link field array and plain URL string.
     *
     * @param int $post_id Post ID.
     * @return array{url:string,target:string,rel:string}
     */
    function inlife_get_post_target_link( $post_id = 0 ) {
        ...
        $redirect = get_field( 'post_redirect_url', $post_id );

        if ( is_array( $redirect ) && ! empty( $redirect['url'] ) ) {
            $link['url'] = trim( (string) $redirect['url'] );

            if ( ! empty( $redirect['target'] ) ) {
                $link['target'] = trim( (string) $redirect['target'] );
            }
        } elseif ( is_string( $redirect ) && '' !== trim( $redirect ) ) {
            /*
             * Backward compatibility if the field was previously a plain URL field.
             */
            $link['url'] = trim( $redirect );

            $new_tab = (bool) get_field( 'post_redirect_new_tab', $post_id );
...
if ( ! function_exists( 'inlife_get_post_target_url' ) ) {
    /**
     * Returns post target URL.
     *
     * @param int $post_id Post ID.
     * @return string
     */
    function inlife_get_post_target_url( $post_id = 0 ) {
        $link = inlife_get_post_target_link( $post_id );

        return ! empty( $link['url'] ) ? $link['url'] : '';
    }
}

if ( ! function_exists( 'inlife_get_post_target_attrs' ) ) {
    /**
     * Returns HTML link attributes for post target.
     *
     * @param int $post_id Post ID.
     * @return array{target:string,rel:string}
     */
    function inlife_get_post_target_attrs( $post_id = 0 ) {
        $link = inlife_get_post_target_link( $post_id );
